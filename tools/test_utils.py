...
class PyTestManifestParser:
    ...
    def get_test_list(self, tf_path, test_regex):
        accepted_formats = [
        ...
        try:
            module_list = self.regex_walk(tf_path, test_regex)
        ...
        try:
            test_list = self.list_tests(module_list, test_regex)
        except Exception as e:
            test_list = [[], []]
            print(
    ...
    def regex_walk(self, dirname, regex_input):
        """
    ...
    def list_tests(self, module_list, regex_input):
        """
    ...
    def read_tests_from_manifest(self, manifestfile, tensorflow_path):
        """
        ...
        list_of_tests = []
        skipped_items = []
        self.imported_files.append(manifestfile)
        with open(manifestfile) as fh:
            ...
            for line in fh.readlines():
                ...
                if curr_section == 'import_section':
                    ...
                    if line in self.imported_files:
                        sys.exit("ERROR: re-import of manifest " + line +
                                 " in " + manifestfile)
                    self.imported_files.append(line)
                    new_runs, new_skips = self.read_tests_from_manifest(
                ...
                if curr_section == 'run_section':
                    list_of_tests.extend(
                        self.get_test_list(tensorflow_path, line)[0])
